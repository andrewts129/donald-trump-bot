...
class TweetBuilder(object):
    ...
    def create_tweet(self, username_to_reply_to):
        ...
        def create_markov():
            ...
            # Downloads the word bank containing all the words (and their part of speech), as well as the two words that
            # precede them
            word_bank = TweetBuilder.create_word_bank()

            # Starts the tweet by adding the first few characters of an actual @realDonaldTrump tweet
            all_starter_letters = TweetBuilder.create_starter_letters()
            chain = random.choice(all_starter_letters)
    ...
    def reply_to_people(self):
        ...
        def get_tweets_to_reply_to():
            ...
            for reply in replies_to_me:
                if not hasattr(reply, 'retweeted_status'):
...
donaldTrumBot = TwitterBot(tweet_builder=tweetBuilder, minutes_between_reply_checks=10,
